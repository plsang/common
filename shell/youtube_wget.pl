...
# clean up the url by translating url encoding and removing unwanted strings
...
# secondary url cleanup after url and title combination
$download =~ s/&+/&/g;
$download =~ s/%2C/,/g;
$download =~ s/%252F/\//g;

# remove multiple itag strings
$download =~ s/&itag=(\d+)// while $download =~ /(&itag=(\d+)).*\1/;
