### cal the similarity matrix 1000 x num_event
...
for event_file in event_files:
    ...
    scores_lsi = numpy.zeros(len(synsets))
    scores_tfidf = numpy.zeros(len(synsets))
    
    for idx, val in enumerate(synsets):
        synset_id = val[0]
        synset_name = val[1]
       
        strvec2 = dictionary.doc2bow(tokenize(synset_name))
    ...
    output_tfidf_file = './eventsims-tfidf/%s-%s.sim.txt' % (event_id, event_names[event_id])
    save_result(output_tfidf_file, scores_tfidf);
...
output_file = '/net/per610a/export/das11f/plsang/trecvidmed14/metadata/event_concept_sim.pickle'    
pickle.dump(scores, open( output_file, "wb" ))    

output_file = '/net/per610a/export/das11f/plsang/trecvidmed14/metadata/event_concept_sim.mat'    
